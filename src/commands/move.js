...
module.exports = {
	...
	async execute(message, args, bot) {
		...
		function sendCurPosition() {
			...
			let attachment = new Discord.Attachment(`../images/forbidden_forest/${user.mazeInfo.curMaze}/Inactive/Forest_${curPos}X.png`, "map.png");

			// If there's a loot bag there set the image as an active tile instead of an inactive one
			if (user.mazeInfo.itemPositions.includes(curPos)) attachment = new Discord.Attachment(`../images/forbidden_forest/${user.mazeInfo.curMaze}/Active/Forest_${curPos}.png`, "map.png");
		...
		function checkTiles() {
			...
			if (!user.mazeInfo.encounterPositions.some(t => fs.readdirSync(`../images/forbidden_forest/${bot.userInfo.get(`${message.guild.id}-${message.author.id}`, "mazeInfo.curMaze")}/Active`).some(f => f.includes(t)))) {
				// Make an array with encounter positions that are
				const encounterTiles = encountersFile.filter(e => e.tiles.some(b => fs.readdirSync(`../images/forbidden_forest/${bot.userInfo.get(`${message.guild.id}-${message.author.id}`, "mazeInfo.curMaze")}/Active`).some(f => f.includes(b))));
			...
			if (!user.mazeInfo.itemPositions.some(i => fs.readdirSync(`../images/forbidden_forest/${bot.userInfo.get(`${message.guild.id}-${message.author.id}`, "mazeInfo.curMaze")}/Active`).some(f => f.includes(i)))) {
				// Make an array with item positions that are
				const itemTiles = Object.values(itemsFile).filter(i => i.tiles.some(b => fs.readdirSync(`../images/forbidden_forest/${bot.userInfo.get(`${message.guild.id}-${message.author.id}`, "mazeInfo.curMaze")}/Active`).some(c => c.includes(b))));
			...
			if (!user.mazeInfo.ambushPositions.some(i => fs.readdirSync(`../images/forbidden_forest/${bot.userInfo.get(`${message.guild.id}-${message.author.id}`, "mazeInfo.curMaze")}/Active`).some(f => f.includes(i)))) {
				// Make an array with ambush positions that are
				const ambushTiles = ambushesFile.filter(e => e.tiles.some(b => fs.readdirSync(`../images/forbidden_forest/${bot.userInfo.get(`${message.guild.id}-${message.author.id}`, "mazeInfo.curMaze")}/Active`).some(f => f.includes(b))));
		...
		async function darkWizard() {
			...
			const darkWizardAttachment = new Discord.Attachment(`../images/forbidden_forest/${user.mazeInfo.curMaze}/Active/Forest_${user.mazeInfo.curPos}.png`, "map.png");
			// Send the attachment
			...
			if (response.content === "1") { // Ignore the wizard
				...
				setTimeout(() => {
					...
					const attachment = new Discord.Attachment(`../images/forbidden_forest/${bot.userInfo.get(`${message.guild.id}-${message.author.id}`, "mazeInfo.curMaze")}/Active/Forest_${bot.userInfo.get(`${message.guild.id}-${message.author.id}`, "mazeInfo.curPos")}.png`, "map.png");
					// Send the attachment
					webhook.send(attachment);
				}, 2000);
		...
		function spawnAmbush() {
			...
			if (ambushInfo.ambushType === "poison") {
				...
				const ambushAttachment = new Discord.Attachment(`../images/forbidden_forest//${user.mazeInfo.curMaze}/Active/Forest_${user.mazeInfo.curPos}.png`, "map.png");
				// Send the attachment
				...
				const ambushPositions = ambushInfo.tiles.filter(i => fs.readdirSync(`../images/forbidden_forest/${bot.userInfo.get(`${message.guild.id}-${message.author.id}`, "mazeInfo.curMaze")}/Active`).some(f => f.toLowerCase().includes(i)));
		...
		function spawnEncounter() {
			...
			const encounterAttachment = new Discord.Attachment(`../images/forbidden_forest//${user.mazeInfo.curMaze}/Active/Forest_${user.mazeInfo.curPos}.png`, "map.png");
			// Send the attachment
			...
			messageCollectors[message.author.id].on("collect", collected => {
				...
				if (health <= 0) {
					...
					const mazeTiles = encounterInfo.tiles.filter(i => fs.readdirSync(`../images/forbidden_forest//${bot.userInfo.get(`${message.guild.id}-${message.author.id}`, "mazeInfo.curMaze")}/Active`).some(f => f.toLowerCase().includes(i)));
