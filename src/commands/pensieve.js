...
module.exports = {
	...
	async execute(message, args, bot) {
		...
		const timeObject = moment.tz("America/Los_Angeles").add(time[0], "days").add(time[1], "hours").add(time[2], "minutes");
		...
		bot.userInfo.push(`${message.guild.id}-${message.author.id}`, object, "reminders");
