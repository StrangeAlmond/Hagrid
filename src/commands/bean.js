module.exports = {
	...
	async execute(message, args, bot) {
		...
		if (bot.userInfo.get(message.author.key, "inventory.beans") <= 0) {
		...
		const uniqueBeans = bot.userInfo.get(message.author.key, "stats.uniqueBeansEaten");

		bot.userInfo.inc(message.author.key, "stats.beansEaten");
		bot.userInfo.dec(message.author.key, "inventory.beans");

		if (!uniqueBeans.includes(flavour)) {
			bot.userInfo.push(message.author.key, flavour, "stats.uniqueBeansEaten");
		}
