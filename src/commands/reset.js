...
module.exports = {
	...
	async execute(message, args, bot) {
		...
		if (!args[0]) {
			return message.channel.send(`Specify a cooldown to reset! Proper Usage: \`${bot.prefix}reset <daily/merit/weekly/study> <@member>\``);
		}

		const user = bot.functions.getUserFromMention(args[1], message.guild) || message.guild.members.cache.get(args[1]);
		if (!user) {
			return message.channel.send(`Specify a member! Proper Usage: \`${bot.prefix}reset <daily/merit/weekly/study> <@member>\``);
		}

		const validCooldowns = ["daily", "merit", "weekly", "study"];
		if (!validCooldowns.includes(args[0])) {
			return message.channel.send(`Invalid cooldown! Proper Usage: \`${bot.prefix}reset <daily/merit/weekly/study> <@member>\``);
		}
