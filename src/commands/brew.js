...
module.exports = {
	...
	async execute(message, args, bot) {
		...
		if (bot.userInfo.get(`${message.guild.id}-${message.author.id}`, "stats.activeEffects").some(e => e.type == "luck")) brewChance = 100;
		...
		if (chance <= brewChance) {
			...
			if (!bot.userInfo.hasProp(`${message.guild.id}-${message.author.id}`, `inventory.${potion.potion}`)) {
				bot.userInfo.set(`${message.guild.id}-${message.author.id}`, 0, `inventory.${potion.potion}`);
			}
