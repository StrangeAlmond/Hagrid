...
module.exports = {
	...
	async execute(message, args, bot) {
		...
		for (let [key, value] of Object.entries(valuesToReset)) {
			db.userInfo.set(`${message.guild.id}-${user.id}`, value, key);
		}
		...
		prestigeItems.forEach(item => {
			db.userInfo.set(`${message.guild.id}-${user.id}`, 1, `inventory.${item}`);
		});
		...
		userData.inventory = db.userInfo.get(message.author.key, "inventory");
		...
		if (userData.inventory.resurrectionStone >= 1 &&
			...
			db.userInfo.push(`${message.guild.id}-${user.id}`, badge.credential, "badges");
		}

		message.channel.send(`${user} has prestiged and been given the ${prestigeItem}!`);

		function elderWand() {
			db.userInfo.math(`${message.guild.id}-${user.id}`, "+", 2, "stats.attack");
			...
			db.userInfo.set(`${message.guild.id}-${user.id}`, elderWandInfo, "wand");
			db.userInfo.set(`${message.guild.id}-${user.id}`, 1, "inventory.elderWand");
		}

		function invisibilityCloak() {
			db.userInfo.set(`${message.guild.id}-${user.id}`, 1, "inventory.invisibilityCloak");
		}

		function resurrectionStone() {
			db.userInfo.set(`${message.guild.id}-${user.id}`, 1, "inventory.resurrectionStone");
			db.userInfo.set(`${message.guild.id}-${user.id}`, null, "cooldowns.lastResurrectionStoneUse");
		}
	},
};
