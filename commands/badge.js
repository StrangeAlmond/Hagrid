...
module.exports = {
  ...
  async execute(message, args, bot) {
    ...
    if (args[0] === "list" || !args[0]) {
    ...
    } else if (args[0] === "add" || args[0] === "grant") {
      ...
      await usersBadges.forEach(badge => {
        badge = usersBadges.find(b => b.credential === badge);
        badgesDescription += `${bot.emojis.get(badge.emojiID)} ${badge.name}\n`;
      });

      if (usersBadges.length > 0) badgesDescription = "You have not earned any badges.";
