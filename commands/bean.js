module.exports = {
  ...
  async execute(message, args, bot) {
    if (!bot.userInfo.hasProp(`${message.guild.id}-${message.author.id}`, "inventory.beans")) {
      ...
      bot.userInfo.set(`${message.guild.id}-${message.author.id}`, [], "inventory.uniqueBeans");
    }
    ...
    const uniqueBeans = bot.userInfo.get(message.author.id, "inventory.uniqueBeans");

    // ++ their "beansEaten" stat, -- their "inventory.beans" stat
    bot.userInfo.inc(`${message.guild.id}-${message.author.id}`, "beansEaten");
    ...
    if (!uniqueBeans.includes(flavour)) {
      bot.userInfo.push(message.author.id, flavour, "inventory.uniqueBeans");
    }
